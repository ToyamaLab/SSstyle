...
//	}
	...
	//process
	 private static ArrayList<DefaultMutableTreeNode> process(String query) {
		...
		/********************************************************************************************/
		//if(tree != null)	new DisplayTree(tree);
		Log.out(tfe);
	...
	private static ArrayList<DefaultMutableTreeNode> getTree(String t, int p) {
    	...
    	try{
			...
			Log.out("nodeCountPlace = "+ nodeCountPlace);
			...
			for(int i=p; i>=0; i--){
				...
				if (!dq && !sq && f == 0) {
					if((c==']' && !(aggregateFunction = isAggregateFunction(t, i))) || (c == '}' && !(deco = isDeco(t, i)))){
						...
						//System.out.println("/////// "+label+" n = "+ (!n.isEmpty()? ""+n.get(nodeCount.get(nodeCountPlace)-1) : "null") +" ///////");
						if(n.isEmpty() && isSingleNode(t, i)){
							//左辺 or SingleNodeの場合
							Log.out("/////// "+label+"左辺 or SingleNode n = "+ (!n.isEmpty()? ""+n.get(nodeCount.get(nodeCountPlace)-1) : "null") +" ///////");
	...
	private static ArrayList<DefaultMutableTreeNode> addNode(
		...
		try{
			...
			Log.out(n1.get(nodeCount.get(nodeCountPlace-1)-1)+" .add "+n2.get(0));
	...
	private static boolean isAggregateFunction(String t, int k) {
		...
		if(t.charAt(k)=='['){
			for(int j=k-1; j>=0; j--){
				...
				if(!Character.isWhitespace(c)){
					Log.out("isAggregateFunction() nextC = "+c);
					if(!isOperator(c) && c!='[' && c!='{'){
						aggregateFunction = false;
						return true;
					}
